...
async function querySingle (allData, isin, beginQueryStr, endQueryStr, lastEvaluatedKey = undefined) {
  ...
  console.log(`Query response: ${JSON.stringify(queryResults, null, '\t')}`)
  ...
  await Promise.all(
    isins
      .map(async (isin) => {
        ...
        do {
          lastEvaluatedKey = querySingle(allData, isin, beginQueryStr, endQueryStr, lastEvaluatedKey)
          if (lastEvaluatedKey === undefined) {
            break
          }
        } while (true)
...
exports.handler = async function (event, context) {
  ...
  try {
    ...
    const {isins, begin, end} = event.queryStringParameters
    const allData = await queryData(isins, begin, end)
